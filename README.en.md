...
1. Edit `src/host/index.js` or `src/client/index.js`
2. `node scripts/build-host.mjs && node scripts/build-client.mjs` (plus `node --check` for syntax)
...
> During development you can also run `src/` directly as a dynamic plugin via the cordis_define/run flow of the "plugin development mode"; both forms share the same source.
...
| Item | Notes |
| --- | --- |
| settings namespace persistence | the `openwiki` namespace registration depends on schemastery, whose dynamic import from the profile node_modules may fail; actual config today uses in-memory state + localStorage, core features unaffected |
